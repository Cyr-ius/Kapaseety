...
class ClusterDetail {
	...
	function toHTML() {
		...
		echo "<div class='row'>";//Host  Buttons List
			...
			$shortname = $fqdn[0];
			if ($value['maintenance']=='True') {$color = 'warning';} else {$color='primary';}
			echo "<button class='btn btn-".$color."' style='margin-left:2px' data-toggle='tooltip' data-placement='top' data-moref='".$value['moref']."' title='Name :".$value['hostname']."\nCPU: ".$value['cpu_usage']."Mhz\nRAM: ".$value['mem_usage']."Mo'><i class='fa fa-building'></i> ".$shortname."</button>";
			}
			echo "</div></div>";
		echo "</div>";		
		echo "<div class='row'>";
		...
		echo "<div class='row'>";//VM  Buttons List
			...
			foreach ($RsltVM as $value) {
				...
				if ($value['prct_cpu'] > 95 or $value['prct_mem']>95){$btncolor="btn-error";}
				echo "<button class='btn $btncolor btn-circle' style='margin-left:2px' data-toggle='tooltip' data-placement='top' data-moref='".$value['vm_moref']."' title='Name :".$value['vmname']."\nCPU: ".$value['prct_cpu']."%\nRAM: ".$value['prct_mem']."%'><i class='fa fa-laptop'></i></button>";
			}
			echo "</div></div>";
		echo "</div>";
		echo "<div class='row'>";
