...
class jsonRPCServer {
	...
	private $errorMessagesFull = array(
		...
		'-32000' => 'The requested extension does not exist or is not available.',
		'-32604' => 'User unknown or Password or Session id incorrect.'
		);
	...
	function PHPErrorHandler($errno, $errstr, $errfile, $errline) {
		...
		$this->ErrorHandler = array (
				...
				'error'	=> array(
					'code'	=> "$errno",
					'message' => (isset($errstr)) ? $errstr : 'internalError',
					
					'data'	=> array(
						'request' => (isset($this->request)) ? $this->request : NULL,
						'extension' => (isset($this->extension)) ? $this->extension : NULL,
						'fullMessage' => ($errstr) ? 'Fatal error on line '.$errline.' in file '.$errfile : 'unknown',
						'debugData' => (isset($errorData)) ?$errorData : NULL,
						)
					)
				);
		echo json_encode($this->ErrorHandler);
		exit;
	}
	...
	public function registerClass($obj){
		foreach($obj as $theclass){
		$this->classes[get_class($theclass)] = $theclass;
		}
		return true;
	}
	/**
	...
	private function validate() {
		...
		try {
			...
			if (!isset($this->classes[$this->extension]) && $this->extension != "system"){
				throw new Exception($this->errorCodes['extensionNotFound']);
			}
			$this->request['method'] = $requestMethod[1];
			if (!isset($this->classes[$this->extension]) && !method_exists($this->classes[$this->extension],$this->request['method']) && $this->extension != "system"){
